class Rupiah {
  ...
  get format(): string {
    let rupiah = "";
    this.setRawNumber = this.checkDecimal(this.rawNumber);
  ...
  private checkDecimal(inputNum: number): number{
    let temp = inputNum.toString();
    let split_temp = temp.split(".");
    if(split_temp.length > 0){
      this.setSuffix = `,${split_temp[1] ?  split_temp[1] : "00"}`;
      return +split_temp[0];
    } else {
      this.setSuffix = ',00';
      return inputNum;
