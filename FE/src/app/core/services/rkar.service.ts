...
export class RkarService {
  ...
  paging(){
    ...
    return this.http.get(`${environment.url}Rkar/paging`,{params: qp}).pipe(map(resp => <any>resp));
  }
  post(postBody: any){
    return this.http.post(`${environment.url}Rkar`, postBody, {observe: 'response'});
  }
  put(postBody: any){
    return this.http.put(`${environment.url}Rkar`, postBody, {observe: 'response'});
  }
  delete(Idrkar: number){
    return this.http.request('DELETE', `${environment.url}Rkar/${Idrkar}`, {observe: 'response'});
  }
}
