...
export class RkasahService {
  ...
  paging(){
    ...
    return this.http.get(`${environment.url}Rkasah/paging`,{params: qp}).pipe(map(resp => <any>resp));
  }
  post(postBody: any){
    return this.http.post(`${environment.url}Rkasah`, postBody, {observe: 'response'});
  }
  put(postBody: any){
    return this.http.put(`${environment.url}Rkasah`, postBody, {observe: 'response'});
  }
  delete(idrkasah: number){
    return this.http.request('DELETE', `${environment.url}Rkasah/${idrkasah}`, {observe: 'response'});
  }
}
