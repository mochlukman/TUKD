...
export class LpjService {
  ...
  getPaging(Idunit: number, Idbend: number){
    ...
    return this.http.get(`${environment.url}Lpj/paging`,{params: qp}).pipe(map(resp => <any>resp));
  }
  post(paramBody: any){
    return this.http.post(`${environment.url}Lpj`, paramBody, {observe:'response'});
  }
  put(paramBody: any){
    return this.http.put(`${environment.url}Lpj`, paramBody, {observe:'response'});
  }
  delete(Idlpj: number){
    return this.http.request('DELETE', `${environment.url}Lpj/${Idlpj}`, {observe: 'response'});
  }
  pengesahan(paramBody: any){
    return this.http.put(`${environment.url}Lpj/pengesahan`, paramBody, {observe:'response'});
  }
}
