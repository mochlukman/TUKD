...
export class RkatapdbService {
  ...
  paging(){
    ...
    return this.http.get(`${environment.url}Rkatapdb/paging`,{params: qp}).pipe(map(resp => <any>resp));
  }
  getNomor(idrka: number){
    return this.http.get(`${environment.url}Rkatapdb/nomor/${idrka}`).pipe(map(resp => <any>resp));
  }
  post(postBody: any){
    return this.http.post(`${environment.url}Rkatapdb`, postBody, {observe: 'response'});
  }
  put(postBody: any){
    return this.http.put(`${environment.url}Rkatapdb`, postBody, {observe: 'response'});
  }
  delete(Idtapd: number){
    return this.http.request('DELETE', `${environment.url}Rkatapdb/${Idtapd}`, {observe: 'response'});
  }
}
