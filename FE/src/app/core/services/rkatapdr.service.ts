...
export class RkatapdrService {
  ...
  paging(){
    ...
    return this.http.get(`${environment.url}Rkatapdr/paging`,{params: qp}).pipe(map(resp => <any>resp));
  }
  getNomor(idrka: number){
    return this.http.get(`${environment.url}Rkatapdr/nomor/${idrka}`).pipe(map(resp => <any>resp));
  }
  post(postBody: any){
    return this.http.post(`${environment.url}Rkatapdr`, postBody, {observe: 'response'});
  }
  put(postBody: any){
    return this.http.put(`${environment.url}Rkatapdr`, postBody, {observe: 'response'});
  }
  delete(Idtapd: number){
    return this.http.request('DELETE', `${environment.url}Rkatapdr/${Idtapd}`, {observe: 'response'});
  }
}