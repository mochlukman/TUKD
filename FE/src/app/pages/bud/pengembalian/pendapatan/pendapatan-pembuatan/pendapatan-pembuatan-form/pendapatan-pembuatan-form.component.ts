...
@Component({
  selector: 'app-pendapatan-pembuatan-form',
  templateUrl: './pendapatan-pembuatan-form.component.html',
  styleUrls: ['./pendapatan-pembuatan-form.component.scss']
})
export class PendapatanPembuatanFormComponent implements OnInit {
  ...
  getPenolakan(){
    ...
    if(this.mode === 'edit'){
      if(this.forms.value.penolakan != null && this.forms.value.penolakan.trim() != '0'){
        this.penolakanSelected = this.listpenolakan.find(w => w.value == this.forms.value.penolakan.trim()).value;
      }
    }
  }
  changePenolakan(e: any){
    this.forms.patchValue({
      penolakan: e.value,
    });
  }
