...
@Component({
  selector: 'app-sp2d-ls-uangmuka-verifikasi-detail-pembebanan',
  templateUrl: './sp2d-ls-uangmuka-verifikasi-detail-pembebanan.component.html',
  styleUrls: ['./sp2d-ls-uangmuka-verifikasi-detail-pembebanan.component.scss']
})
export class Sp2dLsUangmukaVerifikasiDetailPembebananComponent implements OnInit, OnChanges, OnDestroy {
  ...
  get(){
    if(this.Sp2dSelected && this.KegSelected){
      this.loading = true;
      this.listdata = [];
      this.service.gets(this.Sp2dSelected.idspmNavigation.idspd, this.KegSelected.data_id)
        .subscribe(resp => {
          this.listdata = [...resp];
          this.loading = false;
        },(error) => {
