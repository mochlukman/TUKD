    ...
    if (!this.unitSelected) {
      ...
      if(this.reportType == 'A'){
        ...
        this.reportService.execPrint(parameterReport).subscribe((resp) => {
          this.reportService.extractData(resp, 1, `laporan_${this.unitSelected.kdunit}DPA_Belanja`);
        });
      } else if(this.reportType == 'B'){
        if(this.kegSelected){
          let parameterReport = {
            Type: 1,
            FileName: 'dparinc.rpt',
					Params: {
						'@kdtahap': '321',
						'@idunit': this.unitSelected.idunit,
            '@idkeg': this.kegSelected.data_id,
          ...
          this.reportService.execPrint(parameterReport).subscribe((resp) => {
            this.reportService.extractData(resp, 1, `laporan_${this.unitSelected.kdunit}${this.uiKeg.kode}DPA_Belanja_Rincian`);
          });
        } else {
