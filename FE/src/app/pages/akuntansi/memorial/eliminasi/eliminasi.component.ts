...
@Component({
  selector: 'app-eliminasi',
  templateUrl: './eliminasi.component.html',
  styleUrls: ['./eliminasi.component.scss']
})
export class EliminasiComponent implements OnInit, OnDestroy {
  title: string;
  ...
  @ViewChild(EliminasiFormComponent,{static: true}) Form: EliminasiFormComponent;
  ...
  add(){
    ...
    this.service.get('Jbm/bykode', param).subscribe(resp => {
      if(resp.length > 0){
        this.Form.title = 'Tambah Eliminasi';
        this.Form.mode = 'add';
  ...
  update(data: any){
    this.Form.title = 'Ubah Eliminasi';
    this.Form.mode = 'edit';
    this.Form.forms.patchValue({
      idbm : data.idbm,
      idunit : data.idunit,
