...
@Component({
  selector: 'app-surflus-form',
  templateUrl: './surflus-form.component.html',
  styleUrls: ['./surflus-form.component.scss']
})
export class SurflusFormComponent implements OnInit {
  showthis: boolean;
  loadingpost: boolean;
  title: string;
  mode: string;
  ...
  simpan(){
    if(this.forms.valid){
      ...
      if(this.mode === 'add'){
        ...
        this.service.put(`Bktmem`, this.forms.value).subscribe((resp) => {
          if (resp) {
            this.callback.emit({
              edited: true,
              data: resp
