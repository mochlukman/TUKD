...
import { TabPajakTunaiFormComponent } from '../tab-pajak-tunai-form/tab-pajak-tunai-form.component';

@Component({
  selector: 'app-tab-pajak-tunai',
  templateUrl: './tab-pajak-tunai.component.html',
  styleUrls: ['./tab-pajak-tunai.component.scss']
})
export class TabPajakTunaiComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(TabPajakTunaiFormComponent, {static: true}) Form: TabPajakTunaiFormComponent;
  ...
  get(){
    if(this.unitSelected && this.bendSelected){
      ...
      this.listdata = [];
      this.dataSelected = null;
      this.service.gets(this.unitSelected.idunit, this.bendSelected.idbend, '36')
        .subscribe(resp => {
          this.listdata = [];
          if(resp.length > 0){
            this.listdata = [...resp];
  ...
  add(){
    if(this.unitSelected && this.bendSelected){
      this.Form.title = 'Tambah Pajak Belanja Tunai';
      this.Form.mode = 'add';
  ...
  update(e: IBkpajak){
    ...
    this.Form.unitSelected = this.unitSelected;
    this.Form.title = 'Ubah Pajak Belanja Tunai';
    this.Form.mode = 'edit';
    this.Form.showThis = true;
  }
  delete(e: IBkpajak){
