...
import { PelbankBppToBpFormComponent } from './pelbank-bpp-to-bp-form/pelbank-bpp-to-bp-form.component';

@Component({
  selector: 'app-pelbank-bpp-to-bp',
  templateUrl: './pelbank-bpp-to-bp.component.html',
  styleUrls: ['./pelbank-bpp-to-bp.component.scss']
})
export class PelbankBppToBpComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(PelbankBppToBpFormComponent, {static: true}) Form: PelbankBppToBpFormComponent;
  ...
  lookBendahara(){
    if(this.unitSelected){
      ...
      this.Bendahara.gets(this.unitSelected.idunit,'12');
  ...
  get(){
    ...
    if(this.unitSelected && this.bendSelected){
      ...
      this.loading = true;
      this.service.gets(this.unitSelected.idunit,'51', 6, this.bendSelected.idbend)
        .subscribe(resp => {
          if(resp.length > 0){
            this.listdata = [...resp];
          } else {
  ...
  add(){
    if(this.unitSelected && this.bendSelected){
      this.Form.title = 'Tambah Pelimpahan Bank (BPP-BP)';
      this.Form.mode = 'add';
  ...
  update(e: ITbp){
    ...
    this.Form.unitSelected = this.unitSelected;
    this.Form.title = 'Ubah Pelimpahan Bank';
    this.Form.mode = 'edit';
    this.Form.showThis = true;
  }
  delete(e: ITbp){
