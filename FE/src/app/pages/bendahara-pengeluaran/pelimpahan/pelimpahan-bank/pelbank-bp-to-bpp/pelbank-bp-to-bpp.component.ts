...
import { PelbankBpToBppFormComponent } from './pelbank-bp-to-bpp-form/pelbank-bp-to-bpp-form.component';

@Component({
  selector: 'app-pelbank-bp-to-bpp',
  templateUrl: './pelbank-bp-to-bpp.component.html',
  styleUrls: ['./pelbank-bp-to-bpp.component.scss']
})
export class PelbankBpToBppComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(PelbankBpToBppFormComponent, {static: true}) Form: PelbankBpToBppFormComponent;
  ...
  get(){
    ...
    if(this.unitSelected && this.bendSelected){
      ...
      this.loading = true;
      this.service.gets(this.unitSelected.idunit,'37', 6, this.bendSelected.idbend)
        .subscribe(resp => {
          if(resp.length > 0){
            this.listdata = [...resp];
          } else {
  ...
  add(){
    if(this.unitSelected && this.bendSelected){
      this.Form.title = 'Tambah Pelimpahan Bank (BP-BPP)';
      this.Form.mode = 'add';
