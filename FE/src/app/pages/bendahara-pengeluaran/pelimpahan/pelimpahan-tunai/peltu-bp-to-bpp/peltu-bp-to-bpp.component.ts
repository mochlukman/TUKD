...
import { PeltuBpToBppFormComponent } from './peltu-bp-to-bpp-form/peltu-bp-to-bpp-form.component';

@Component({
  selector: 'app-peltu-bp-to-bpp',
  templateUrl: './peltu-bp-to-bpp.component.html',
  styleUrls: ['./peltu-bp-to-bpp.component.scss']
})
export class PeltuBpToBppComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(PeltuBpToBppFormComponent, {static: true}) Form: PeltuBpToBppFormComponent;
  ...
  get(){
    ...
    if(this.unitSelected && this.bendSelected){
      ...
      this.loading = true;
      this.service.gets(this.unitSelected.idunit,'38', 6, this.bendSelected.idbend)
        .subscribe(resp => {
          if(resp.length > 0){
            this.listdata = [...resp];
          } else {
  ...
  add(){
    if(this.unitSelected && this.bendSelected){
      this.Form.title = 'Tambah Pelimpahan Tunai (BP-BPP)';
      this.Form.mode = 'add';
