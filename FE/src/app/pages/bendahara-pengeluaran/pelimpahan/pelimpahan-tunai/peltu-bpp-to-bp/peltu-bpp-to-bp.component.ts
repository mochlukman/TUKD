...
import { PeltuBppToBpFormComponent } from './peltu-bpp-to-bp-form/peltu-bpp-to-bp-form.component';

@Component({
  selector: 'app-peltu-bpp-to-bp',
  templateUrl: './peltu-bpp-to-bp.component.html',
  styleUrls: ['./peltu-bpp-to-bp.component.scss']
})
export class PeltuBppToBpComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(PeltuBppToBpFormComponent, {static: true}) Form: PeltuBppToBpFormComponent;
  ...
  get(){
    ...
    if(this.unitSelected && this.bendSelected){
      ...
      this.loading = true;
      this.service.gets(this.unitSelected.idunit,'52', 6, this.bendSelected.idbend)
        .subscribe(resp => {
          if(resp.length > 0){
            this.listdata = [...resp];
          } else {
  ...
  add(){
    if(this.unitSelected && this.bendSelected){
      this.Form.title = 'Tambah Pelimpahan Tunai (BPP-BP)';
      this.Form.mode = 'add';
