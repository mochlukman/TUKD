...
import { TolakUpCheckComponent } from './tolak-up-check/tolak-up-check.component';
import { TolakUpFormComponent } from './tolak-up-form/tolak-up-form.component';

@Component({
  selector: 'app-tolak-up',
  templateUrl: './tolak-up.component.html',
  styleUrls: ['./tolak-up.component.scss']
})
export class TolakUpComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @ViewChild(TolakUpFormComponent, {static: true}) Form : TolakUpFormComponent;
  @ViewChild(TolakUpCheckComponent, {static: true}) Checks: TolakUpCheckComponent;
        ...
        this.service.penolakan(postBody).subscribe(
          (resp) => {
            if (resp.ok) {
              ...
              this.notif.success('Penolakan Berhasil Dibatalkan');
            }
          }, (error) => {
            if (Array.isArray(error.error.error)) {
              for (var i = 0; i < error.error.error.length; i++) {
