...
@Component({
  selector: 'app-pergeseran-uang-form',
  templateUrl: './pergeseran-uang-form.component.html',
  styleUrls: ['./pergeseran-uang-form.component.scss']
})
export class PergeseranUangFormComponent implements OnInit {
  showThis: boolean;
  ...
  getStattrs(){
    ...
    this.stattrsService.getBylist('33,34')
      .subscribe(resp => {
  ...
  changeStattrs(e: any){
    if(e.value){
      ...
      this.forms.patchValue({
        kdstatus:  e.value,
        nobuku: `XXXXX/BK-${label_split[0].trim()}/${this.unitSelected.kdunit}/${this.userInfo.NmTahun}`,
      });
    } else {
      this.forms.patchValue({
        kdstatus: '',
