...
@Component({
  selector: 'app-pembuatan-bpk-pungutan-pajak-form',
  templateUrl: './pembuatan-bpk-pungutan-pajak-form.component.html',
  styleUrls: ['./pembuatan-bpk-pungutan-pajak-form.component.scss']
})
export class PembuatanBpkPungutanPajakFormComponent implements OnInit {
  showThis: boolean;
  ...
  constructor(
    ...
    this.forms = this.fb.group({
      idbpkpajak : 0,
      idbpk : [0, [Validators.required, Validators.min(1)]],
      uraian : '',
      nomor : '',
      tanggal : null,
  ...
  getStattrs(){
    ...
    this.listStattrs = [];
    this.stattrsService.getBylist('35')
      .subscribe(resp => {
        if(resp.length > 0){
          resp.forEach(x => {
