...
@Component({
  selector: 'app-pengesahan-form',
  templateUrl: './pengesahan-form.component.html',
  styleUrls: ['./pengesahan-form.component.scss'],
  providers: [ InputRupiahPipe ]
})
export class PengesahanFormComponent implements OnInit {
  showThis: boolean;
  ...
  constructor(
    ...
    this.forms = this.fb.group({
      ...
      idxkode : 1,
      tglsts : {value: null, disabled: true},
  ...
  getStattrs(){
    ...
    this.stattrsService.getBylist('13,17,19,78,79')
      .subscribe(resp => {
  ...
  simpan(){
    if(this.forms.valid){
      ...
      this.forms.controls['tglsts'].enable();
      this.forms.patchValue({
      ...
      if(this.mode === 'edit'){
        this.service.pengesahan(this.forms.value).subscribe((resp) => {
          if (resp.ok) {
            ...
            this.onHide();
            this.notif.success('Data berhasil disahkan');
          }
  ...
  onHide(){
    ...
    this.forms.controls['tglsts'].disable();
    this.msg = [];
    this.loading_post = false;
