...
import { BkuBelanjaFormComponent } from './bku-belanja-form/bku-belanja-form.component';

@Component({
  selector: 'app-bku-belanja',
  templateUrl: './bku-belanja.component.html',
  styleUrls: ['./bku-belanja.component.scss']
})
export class BkuBelanjaComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(BkuBelanjaFormComponent, {static: true}) Form: BkuBelanjaFormComponent;
  constructor(
      ...
      this.service.generateNoBku(this.formFilter.value.idunit, this.formFilter.value.idbend)
        .subscribe(resp => {
          if(resp.nobku){
            ...
            this.Form.title = 'Tambah BKU Belanja';
            this.Form.mode = 'add';
  ...
  delete(e: IBku){
    this.notif.confir({
			...
			accept: () => {
				this.service.delete(e.nobkuskpd, 'bpk').subscribe(
					(resp) => {
						if (resp.ok) {
