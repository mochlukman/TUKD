...
import { BkuPanjarFormComponent } from './bku-panjar-form/bku-panjar-form.component';

@Component({
  selector: 'app-bku-panjar',
  templateUrl: './bku-panjar.component.html',
  styleUrls: ['./bku-panjar.component.scss']
})
export class BkuPanjarComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(BkuPanjarFormComponent, {static: true}) Form: BkuPanjarFormComponent;
  constructor(
      ...
      this.service.generateNoBku(this.formFilter.value.idunit, this.formFilter.value.idbend)
        .subscribe(resp => {
          if(resp.nobku){
            ...
            this.Form.title = 'Tambah BKU Panjar';
            this.Form.mode = 'add';
  ...
  delete(e: IBku){
    this.notif.confir({
			...
			accept: () => {
				this.service.delete(e.nobkuskpd, 'panjar').subscribe(
					(resp) => {
						if (resp.ok) {
