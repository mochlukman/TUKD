...
import { BkuPajakFormComponent } from './bku-pajak-form/bku-pajak-form.component';

@Component({
  selector: 'app-bku-pajak',
  templateUrl: './bku-pajak.component.html',
  styleUrls: ['./bku-pajak.component.scss']
})
export class BkuPajakComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(BkuPajakFormComponent, {static: true}) Form: BkuPajakFormComponent;
  constructor(
      ...
      this.service.generateNoBku(this.formFilter.value.idunit, this.formFilter.value.idbend)
        .subscribe(resp => {
          if(resp.nobku){
            ...
            this.Form.title = 'Tambah BKU Pajak';
            this.Form.mode = 'add';
  ...
  delete(e: IBku){
    this.notif.confir({
			...
			accept: () => {
				this.service.delete(e.nobkuskpd, 'pajak').subscribe(
					(resp) => {
						if (resp.ok) {
