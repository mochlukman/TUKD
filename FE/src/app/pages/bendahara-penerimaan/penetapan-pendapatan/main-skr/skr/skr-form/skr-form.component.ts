...
@Component({
  selector: 'app-skr-form',
  templateUrl: './skr-form.component.html',
  styleUrls: ['./skr-form.component.scss'],
  providers: [ InputRupiahPipe ]
})
export class SkrFormComponent implements OnInit {
  showThis: boolean;
  ...
  getStattrs(){
    ...
    this.stattrsService.getBylist('73,77')
      .subscribe(resp => {
  ...
  changeStattrs(e: any){
    if(e.value){
      ...
      this.forms.patchValue({
        kdstatus:  e.value,
        noskp: `XXXXX/SKR/${this.unitSelected.kdunit}/${this.forms.value.jabbend}/${this.userInfo.NmTahun}`,
      });
    } else {
      this.forms.patchValue({
        kdstatus: '',
