...
@Component({
  selector: 'app-sts-pengajuan-form',
  templateUrl: './sts-pengajuan-form.component.html',
  styleUrls: ['./sts-pengajuan-form.component.scss']
})
export class StsPengajuanFormComponent implements OnInit {
  showThis: boolean;
  ...
  getStattrs(){
    ...
    this.stattrsService.getBylist('66')
      .subscribe(resp => {
  ...
  lookSkp(){
    if(this.forms.value.kdstatus != ''){
      this.Skp.title = 'Pilih SKP';
      this.Skp.gets(this.forms.value.idunit, this.forms.value.idbend, this.forms.value.kdstatus, this.forms.value.idxkode);
  ...
  callBackSkp(e: ISkp){
    this.skpSelected = e;
    this.uiSkp = {
      kode: this.skpSelected.noskp, 
      nama: this.skpSelected.tglskp
    };
    this.forms.patchValue({
