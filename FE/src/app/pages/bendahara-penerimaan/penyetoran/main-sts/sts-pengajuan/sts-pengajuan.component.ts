...
import { StsPengajuanFormComponent } from './sts-pengajuan-form/sts-pengajuan-form.component';

@Component({
  selector: 'app-sts-pengajuan',
  templateUrl: './sts-pengajuan.component.html',
  styleUrls: ['./sts-pengajuan.component.scss']
})
export class StsPengajuanComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(StsPengajuanFormComponent, {static: true}) Form: StsPengajuanFormComponent;
  ...
  get(){
    ...
    if(this.unitSelected && this.bendSelected){
      ...
      this.loading = true;
      this.service.gets(this.unitSelected.idunit, this.bendSelected.idbend, '66', 1, )
        .subscribe(resp => {
          if(resp.length > 0){
            this.listdata = [...resp];
          } else {
