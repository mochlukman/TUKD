...
import { BkuPenyetoranFormComponent } from './bku-penyetoran-form/bku-penyetoran-form.component';

@Component({
  selector: 'app-bku-penyetoran',
  templateUrl: './bku-penyetoran.component.html',
  styleUrls: ['./bku-penyetoran.component.scss']
})
export class BkuPenyetoranComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(BkuPenyetoranFormComponent, {static: true}) Form: BkuPenyetoranFormComponent;
  constructor(
  ...
  delete(e: IBkuPenerimaan){
    this.notif.confir({
			...
			accept: () => {
				this.service.delete(e.nobkuskpd, 'sts').subscribe(
					(resp) => {
						if (resp.ok) {
