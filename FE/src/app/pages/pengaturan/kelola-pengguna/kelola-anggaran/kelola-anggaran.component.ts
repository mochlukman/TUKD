...
@Component({
  selector: 'app-kelola-anggaran',
  templateUrl: './kelola-anggaran.component.html',
  styleUrls: ['./kelola-anggaran.component.scss']
})
export class KelolaAnggaranComponent implements OnInit, OnDestroy {
  ...
  get(){
    if(this.unitSelected){
      ...
      this.loading = true;
      this.service.getByGroupId('9,14',this.unitSelected.idunit)
        .subscribe(resp => {
          this.listdata = [];
          if(resp.length > 0){
            this.listdata = [...resp];
  ...
  openApproved(e: IWebUser){
    this.Form.title = `Form Persetujuan Pengguna Anggaran`;
