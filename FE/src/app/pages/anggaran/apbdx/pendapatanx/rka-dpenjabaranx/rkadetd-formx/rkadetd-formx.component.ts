...
@Component({
  selector: 'app-rkadetd-formx',
  templateUrl: './rkadetd-formx.component.html',
  styleUrls: ['./rkadetd-formx.component.scss'],
  providers: [InputRupiahPipe]
})
export class RkadetdFormxComponent implements OnInit {
  ...
  constructor(
    ...
    this.forms = this.fb.group({
      idrkadetd: 0,
      idrkad: [0, [Validators.required, Validators.min(1)]],
  ...
  simpan() {
    if (this.forms.valid) {
      ...
      if (this.mode === 'add') {
        if(this.forms.value.idrkadetdduk == null){
          this.forms.patchValue({
            idrkadetdduk: 0
          });
        }
